...
const activationFuncs: funcsLib = {
	"abs": Math.abs,
	"tanh": Math.tanh,
	"sigmoid": (x: number) => 1 / (1 + Math.exp(-x)),
	"relu": (x: number) => Math.max(0, x),
	"leaky_relu": (x: number) => x >= 0 ? x : 0.01 * x, // You can adjust the slope of the negative part
	"nothing": (x: number) => x
};
...
class Neuron {
	public bias: number
	public activationFunc: string
	public weights: number[]
	constructor(amountOfWeights: number, bias: number = 0) {
		this.bias = bias
	...
	ResetWeights(amountOfNInPrev: number) {
		this.weights = []
		for (let i: number = 0; i < amountOfNInPrev; i++) {
			this.weights.push(0)
		}
	}
	...
	SetActivationFunc(activationFunc: string) {
	...
	CalculateOutput(inputs: number[]) {
		let activation: number = this.bias
...
class Layer {
	...
	GetNeuron(neuron: number): Neuron {
		if (neuron < 0) {
	...
	GetNeuronsBiases(): number[] {
		return this.neurons.map(n => n.bias)
	}

	GetNeuronsActivationFuncs(): string[] {
		return this.neurons.map(n => n.activationFunc)
	}

	GetNeuronsWeights(neuron: number): number[] {
		if (neuron < 0) {
			throw new Error("Neuron index can't be less than 0")
		}
	...
	AddNeuron() {
		...
		this.next.neurons.forEach((neuron: Neuron) => {
			neuron.AddWeight()
		})
	}
...
class NN {
	...
	GetNeuronsActivationFuncs(layer: number) {
		if (layer < 0) {
			throw new Error("Layer's index can't be less than 0")
		}
	...
	GetNeuronsWeights(layer: number, neuron: number) {
		if (layer < 0) {
			throw new Error("Layer's index can't be less than 0")
		}
	...
	AddLayer(index: number) {
		let amountOfNInPrev: number
		if (index != 0) {
			amountOfNInPrev = this.layers[index - 1].neurons.length
		} else {
			amountOfNInPrev = this.amountOfNInIL
		}
		const newLayer: Layer = new Layer(this.layers[index], amountOfNInPrev)
	...
	SetWeight(layer: number, neuron: number, weight: number, val: number) {
		this.layers[layer].neurons[neuron].SetWeight(weight, val)
	}

	SetBias(layer: number, neuron: number, val: number) {
	...
	RemoveNeuron(layer: number, neuron: number) {
		this.layers[layer].RemoveNeuron(neuron)
	}

	SetActivationFunc(layer: number, neuron: number, activationFunc: string) {
...
function ParseCMD(cmd: string, nn: NN) {
